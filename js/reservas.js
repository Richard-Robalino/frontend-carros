...
async function loadReservas() {
  try {
    const res = await fetch('http://localhost:4000/api/reservas', { headers: authHeader() });
    const data = await res.json();
...
reservaForm.addEventListener('submit', async e => {
  ...
  try {
    const resExist = await fetch('http://localhost:4000/api/reservas', { headers: authHeader() });
  ...
  try {
    if (editReservaId) {
      const res = await fetch(`http://localhost:4000/api/reservas/${editReservaId}`, {
        method: 'PUT',
...
async function editarReserva(id) {
  try {
    const res = await fetch(`http://localhost:4000/api/reservas/${id}`, { headers: authHeader() });
